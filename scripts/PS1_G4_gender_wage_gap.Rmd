...
# --- Picos por género ---
db= db%>%
  mutate(bin_female=as.factor(db$bin_female))
class(db$bin_female)

co <- coef(model_equality3interaction)

b2 <- co["age"]
b3 <- if ("age2" %in% names(co)) co["age2"]

# nombres de interacciones pueden variar si bin_female es factor vs numérico:
d2 <- co[intersect(c("age:bin_female"), names(co))]
d3 <- co[intersect(c("age2:bin_female","I(age^2):bin_female"), names(co))]
