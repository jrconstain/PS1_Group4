...
db = db_base |> select(Log_Total_hour_salary, ingtot, y_ingLab_m, y_salary_m, y_salarySec_m, y_ingLab_m_ha, totalHoursWorked, hoursWorkUsual, hoursWorkActualSecondJob, age, sex, estrato1, maxEducLevel, college, edu_years, exp, oficio, relab, formal, microEmpresa, regSalud, sizeFirm, p6240, ocu, dsi)
...
form_A1 <- Log_Total_hour_salary ~ bin_female + age + age2 + maxEducLevel + exp + exp2+ formal + sizeFirm + relab + oficio
...
ggplot(df_errores, aes(x = error)) +
  ...
  geom_vline(xintercept = Mean_errorsA2VS, color = "black", linetype = "dashed", size = 1) +
  geom_vline(xintercept = Mean_errorsA2VS + 2 * sd_errorsA2VS, color = "darkblue", linetype = "dashed", size = 1) +
  geom_vline(xintercept = Mean_errorsA2VS - 2 * sd_errorsA2VS, color = "darkblue", linetype = "dashed", size = 1) +
